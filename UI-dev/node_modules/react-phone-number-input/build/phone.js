...
exports.parse_plaintext_local = parse_plaintext_local;
...
function validate(plaintext_international, format) {
	...
	if (plaintext_international.length !== '+'.length + format.country.length + digits_in_international_phone_number_template(format, plaintext_international)) {
		return false;
	}
...
//
function parse_plaintext_international(formatted, format, has_trunk_prefix) {
	// The input digits
	...
	var value = plaintext_international(digits, format, has_trunk_prefix);
...
// This function is used in <input/> to parse text into `value`
//
function parse_plaintext_local(formatted, format, with_trunk_prefix) {
	// Sanity check
	if (formatted[0] === '+') {
	...
	var has_trunk_prefix = true;

	// Trim excessive phone number digits
	// (this is used in <input/>)
	var value = local_phone_digits(formatted, format, has_trunk_prefix);
...
//
function local_phone_digits(value, format, has_trunk_prefix) {
...
function format_local(value, format, with_trunk_prefix) {
	...
	if (with_trunk_prefix === false) {
		if (value[0] === '+') {
			...
			var plaintext_local_value = void 0;
...
function format_international(value, format) {
	...
	var number = local_to_international_style(format_local(value, format, with_trunk_prefix));
...
//
function digits_in_local_phone_number_template(format, plaintext_local, has_trunk_prefix) {
	var digits = has_trunk_prefix ? trim_trunk_prefix(plaintext_local, format) : plaintext_local;
	var _template = template(format, plaintext_local);
	var regular_expression = has_trunk_prefix === false ? /[A-z]/g : /[0-9A-z]/g;
...
function digits_in_international_phone_number_template(format, plaintext_international) {
	...
	var including_trunk_prefix = false;
...
//                  ^
//
function index_in_template(digit_index, format, digits, with_trunk_prefix, international) {
...
function plaintext_international(plaintext, format, has_trunk_prefix) {
	if (!plaintext) {
		return '';
	}
...
//
function plaintext_local(plaintext, format, has_trunk_prefix) {
	if (!plaintext) {
		return '';
	}
...
//
function trim_trunk_prefix(plaintext_local, format) {
